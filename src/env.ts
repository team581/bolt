...
const allowMissingInGitHubActions = <T>(options: ReporterOptions<T>) => {
	if (!baseEnv.CI) {
		return defaultReporter(options);
	}

	const errors = Object.fromEntries(
		(Object.entries(options.errors) as [keyof T, Error][]).filter(([, error]) => !(error instanceof EnvMissingError)),
	);

	return defaultReporter({ ...options, errors });
};
...
export const env = cleanEnv(
	...
	{ reporter: allowMissingInGitHubActions },
);
